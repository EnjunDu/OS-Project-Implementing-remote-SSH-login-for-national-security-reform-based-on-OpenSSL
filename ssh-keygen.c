...
/*
	此处的代码涉及了RSA算法的函数，如果我们要改造的话，这里面的参数都得改成sm2算法
*/
...
type_bits_valid(int type, const char *name, u_int32_t *bitsp)
{
	...
	if (*bitsp == 0) {
		...
		case SM2:
			if (name != NULL &&
			    (nid = sshkey_sm2_from_name(name)) > 0)
				*bitsp = sshkey_curve_nid_to_bits(nid);
			if (*bitsp == 0)
				*bitsp = DEFAULT_BITS_SM2;
			break;
		case KEY_ECDSA:
			if (name != NULL &&
	...
	case KEY_RSA:
		...
		break;
	case KEY_SM2:
		if(sshkey_sm2_bits_to_nid(*bitsp) == -1)
			fatal("Invalid SM2 key length: valid lengths are "
			    "256 bits");
	case KEY_ECDSA:
...
#ifdef OPENSSL_HAS_ECC
	case KEY_SM2:
		if (!PEM_write_EC_PUBKEY(stdout, k->sm2))
...
do_convert_from_pkcs8(struct sshkey **k, int *private)
{
	...
	case EVP_PKEY_SM2:
		if ((*k = sshkey_new(KEY_UNSPEC)) == NULL)
			fatal("sshkey_new failed");
		(*k)->type = KEY_ECDSA;
		(*k)->sm2 = EVP_PKEY_get1_EC_KEY(pubkey);
		(*k)->sm2_nid = sshkey_sm2_key_to_nid((*k)->sm2);
		break;
	case EVP_PKEY_EC:
		if ((*k = sshkey_new(KEY_UNSPEC)) == NULL)
			fatal("sshkey_new failed");
...
#ifdef OPENSSL_HAS_ECC
		case KEY_SM2:
			ok = PEM_write_ECPrivateKey(stdout, k->sm2, NULL,
			    NULL, 0, NULL, NULL);
			break;
		case KEY_ECDSA:
